    ...
    <div class="mx-auto container py-8 mb-55">
        <div class="flex flex-col mx-auto items-center justify-center bg-white max-w-sm rounded-lg p-8 gap-8">
            ...
            <form action="{{route('registerPost')}}" method="post" class="flex flex-col gap-4">
                @csrf
                 <input type="text" name="name" placeholder="Username" class="px-4 py-2 border rounded-sm">
                     @error('name')
                     value="{{old('name')}}"
                ...
                <span class="text-[14px] opacity-40">The password must contain at least 7 letters.</span>
            </form>
        </div>
    </div>
@endsection